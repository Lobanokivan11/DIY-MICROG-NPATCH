...
TARGET_CLASS_PATH="top/nkbe/npatch/LSPApplication"

unzip -q "$APK_IN" "*.dex" -d tmp_dex

FOUND_DEX=""
for dex in tmp_dex/*.dex; do
    if baksmali list classes "$dex" | grep -q "^${TARGET_CLASS_DOT}$"; then
        FOUND_DEX=$(basename "$dex")
        echo "Class Found in: $FOUND_DEX"
        break
    fi
done

if [ -z "$FOUND_DEX" ]; then
    echo "Error: dex containing $TARGET_CLASS_DOT not found!"
    rm -rf tmp_dex
    exit 1
fi
...
if [ ! -f "$SMALI_FILE" ]; then
    echo "Error: Smali file not found at $SMALI_FILE"
